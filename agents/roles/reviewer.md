...
5. **Merge & closeout**
    ...
    **Completion gate**: this turn is incomplete unless you either (a) merge and close out the task, or (b) explicitly report what prevented merge/closeout and set an appropriate non-clean status.
    - Add a short reflection entry to `agents/context/lessons.md` in the format `[T-00X][reviewer]: <lesson learned>` (append-only; one line per lesson).
...
- Append task reflection lessons to `agents/context/lessons.md` using `[T-00X][reviewer]: ...` entries.
