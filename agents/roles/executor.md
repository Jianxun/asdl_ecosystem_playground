...
5. **Closeout**
    ...
    - Add a short reflection entry to `agents/context/lessons.md` in the format `[T-00X][executor]: <lesson learned>` (append-only; one line per lesson).
...
- Append task reflection lessons to `agents/context/lessons.md` using `[T-00X][executor]: ...` entries.
